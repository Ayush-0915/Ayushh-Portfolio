...
const showcaseMembers = [
    ...
    ...portfolioData.experiences.filter(exp => exp.id === 'exp-ai').map(exp => ({
        id: exp.id,
    ...
    // 2. ML Engineer
    ...portfolioData.experiences.filter(exp => exp.id === 'exp-ml').map(exp => ({
        id: exp.id,
    ...
    ...portfolioData.experiences.filter(exp => exp.id === 'exp-open-source').map(exp => ({
        id: exp.id,
        name: "Open Source Contributor",
        role: exp.position,
        description: exp.description,
        period: "January 2024 - Present",
        image: "https://images.unsplash.com/photo-1527689368864-3a821dbccc34?q=80&w=400&auto=format&fit=crop",
        social: exp.externalLink ? { website: exp.externalLink } : undefined
    })),
    // 4. Lead Creator
    ...portfolioData.experiences.filter(exp => exp.id === 'exp-products').map(exp => ({
        id: exp.id,
        name: "CareerNova AI",
        role: exp.position,
        description: exp.description,
    ...
    // 5. AI Researcher
    ...portfolioData.experiences.filter(exp => exp.id === 'exp-research').map(exp => ({
        id: exp.id,
        name: "AI Researcher",
        role: exp.position,
        description: exp.description,
                ...
                <div className="relative z-10">
                    ...
                    >
                        <h2 className="text-[32px] sm:text-[48px] md:text-[64px] lg:text-[76px] xl:text-[88px] font-bold tracking-tight leading-[0.92] text-zinc-900 dark:text-white transition-colors">
                            ...
                            <span className="text-zinc-900 dark:text-white">{t('leadIn.because')}</span> <span className="font-serif italic font-normal text-zinc-900 dark:text-white lowercase opacity-90">{t('leadIn.engineering')}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 border-t border-zinc-100 dark:border-zinc-900 pt-8 md:pt-12">
                        ...
                        <div className="md:col-span-7 flex flex-col sm:flex-row gap-8 text-[13px]">
                            <div className="flex-1 space-y-3">
                                <span className="text-zinc-800 dark:text-zinc-200 font-bold uppercase tracking-widest block border-b border-zinc-100 dark:border-zinc-900 pb-3">{t('leadIn.focusTitle')}</span>
                            ...
                            <div className="flex-1 space-y-3 flex flex-col">
                                <span className="text-zinc-800 dark:text-zinc-200 font-bold uppercase tracking-widest block border-b border-zinc-100 dark:border-zinc-900 pb-3">{t('leadIn.buildingTitle')}</span>
                                <p className="text-zinc-500 leading-relaxed whitespace-pre-line">
                                    {t('leadIn.integration')}
                                </p>
                                <div className="mt-6 md:mt-auto pt-4 flex flex-col">
                                    <span className="text-3xl lg:text-4xl font-signature text-zinc-900 dark:text-white/90 block">{t('leadIn.signature')}</span>
                                    <span className="text-[11px] text-zinc-400 dark:text-zinc-500 font-mono tracking-widest uppercase block mt-1">{t('leadIn.signatureTitle')}</span>
                                </div>
                            </div>
                        </div>
                    </div>
