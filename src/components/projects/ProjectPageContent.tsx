        ...
        <div className="min-h-screen bg-background text-foreground pb-24 pt-24 sm:pt-32">
                ...
                >
                    ...
                    <div className="w-full">
                        ...
                        {project.slug === 'nexus-ai' ? (
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-6 border bg-emerald-500/10 dark:bg-emerald-500/5 border-emerald-500/25 text-emerald-600 dark:text-emerald-400">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                ACTIVE DEVELOPMENT
                            </div>
                        ) : (project.slug === 'creditwise' || project.slug === 'careernova' || project.slug === 'uber-trips-analysis') ? (
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-6 border bg-emerald-500/10 dark:bg-emerald-500/5 border-emerald-500/25 text-emerald-600 dark:text-emerald-400">
                                <span className="mr-0.5 font-sans">✓</span> COMPLETED PROJECT
                            </div>
                        ) : (
                        ...
                        {project.subtitle && (
                            <p className="text-lg md:text-xl text-foreground/80 dark:text-white/80 font-medium max-w-3xl mb-6 italic leading-relaxed">
                                {project.subtitle}
                            </p>
                        )}
            ...
            <div className="container max-w-7xl mx-auto px-6 mb-20">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4 border-y border-black/20 dark:border-border/40 py-8">
                    ...
                    <div className="flex flex-col gap-2">
                        <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold flex items-center gap-2">
                            <Layers className="w-3.5 h-3.5" /> {t('metadata.techStack')}
                        </span>
                        <span className="font-bold text-foreground truncate" title={project.techStack.join(', ')}>
                            {project.slug === 'nexus-ai' || project.slug === 'careernova' ? '8+ Technologies' : project.slug === 'uber-trips-analysis' ? '7+ Technologies' : project.slug === 'creditwise' ? '6+ Technologies' : t('metadata.techStackValue', { count: project.techStack.length })}
                        </span>
                    </div>
                </div>
            </div>
                        ...
                        {project.outcome && (
                            <section id="outcome">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h2 className="text-2xl font-bold text-foreground">Project Outcome</h2>
                                </div>
                                <div className="prose prose-lg dark:prose-invert prose-emerald max-w-none text-zinc-600 dark:text-muted-foreground leading-relaxed font-light">
                                    <p>{project.outcome}</p>
                                </div>
                            </section>
                        )}
                    ...
                    <div className="lg:col-span-4 relative">
                        <div className="sticky top-20 space-y-8">
                            ...
                            <div className="p-6 rounded-2xl bg-white dark:bg-secondary/5 border border-black/20 dark:border-white/10 backdrop-blur-sm shadow-sm dark:shadow-none">
                                ...
                                <div className={cn("grid gap-3", (project.demoUrl && project.demoUrl !== '#' && project.repoUrl && project.repoUrl !== '#') ? "grid-cols-2" : "grid-cols-1")}>
                                    {project.demoUrl && project.demoUrl !== '#' && (
                                        <motion.a
                                            ...
                                            className={cn(
                                                "flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm transition-all text-center w-full",
                                                project.demoUrl === '#'
                                                    ? "bg-zinc-800 text-zinc-500 cursor-not-allowed opacity-50 border border-white/5"
                                                    : "bg-foreground text-background hover:opacity-90 shadow-lg shadow-black/10 dark:shadow-black/30"
                                            )}
                                    ...
                                    {project.repoUrl && (
                                        <motion.a
                                            ...
                                            className="flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm transition-all text-center w-full bg-secondary/20 dark:bg-secondary/10 border border-black/15 dark:border-white/10 text-foreground hover:bg-secondary/30 dark:hover:bg-secondary/20 shadow-sm"
                            ...
                            {(project.version || project.platform || project.projectType || project.category || project.deployment) && (
                                <div className="p-6 rounded-2xl bg-white dark:bg-secondary/5 border border-black/20 dark:border-white/10 backdrop-blur-sm shadow-sm dark:shadow-none">
                                    <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6 pb-2 border-b border-black/10 dark:border-white/5 flex items-center gap-2">
                                        <Layers className="w-3.5 h-3.5" /> Project Specifications
                                    </h3>
                                    <div className="space-y-4 text-sm">
                                        ...
                                        <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                            <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">
                                                {project.status === 'completed' ? 'Completed' : 'Started'}
                                            </span>
                                        ...
                                        {project.version && (
                                            <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                                <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">Current Version</span>
                                                <span className="font-bold text-foreground bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded text-xs font-mono">
                                                    {project.version}
                                                </span>
                                            </div>
                                        )}
                                        {project.platform && (
                                            <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                                <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">Platform</span>
                                                <span className="font-bold text-foreground">{project.platform}</span>
                                            </div>
                                        )}
                                        {project.projectType && (
                                            <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                                <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">Type</span>
                                                <span className="font-bold text-foreground text-right text-xs">{project.projectType}</span>
                                            </div>
                                        )}
                                        {project.category && (
                                            <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                                <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">Category</span>
                                                <span className="font-bold text-foreground text-right text-xs">{project.category}</span>
                                            </div>
                                        )}
                                        {project.deployment && (
                                            <div className="flex justify-between items-center py-1 border-t border-black/5 dark:border-white/5">
                                                <span className="text-muted-foreground text-xs font-mono uppercase tracking-wider">Deployment</span>
                                                <span className="font-bold text-foreground text-right text-xs bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded font-mono text-[11px] uppercase tracking-wider">{project.deployment}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <ul className="space-y-3 text-sm text-muted-foreground">
                                    ...
                                    {project.outcome && <li onClick={() => document.getElementById('outcome')?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-foreground cursor-pointer transition-colors hover:translate-x-1 duration-200 block">• Project Outcome</li>}
