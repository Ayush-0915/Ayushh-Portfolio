...
export async function GET() {
  const username = process.env.KAGGLE_USERNAME || "Ayush-0915";
  ...
  if (!apiToken) {
    const mockDatasets = [
      { title: "Netflix Userbase Dataset", votes: 4, views: 240, updated: "2025-08-01", url: "https://www.kaggle.com", category: "Dataset", usability: 10.0, size: "1.2 MB" },
      { title: "Uber Rides Analysis Log", votes: 2, views: 180, updated: "2025-07-15", url: "https://www.kaggle.com", category: "Dataset", usability: 9.4, size: "4.5 MB" }
    ];
    const mockModels = [
      { title: "RAG Core Vector Pipeline", category: "Model", updated: "2026-03-20", votes: 2 }
    ];
    return NextResponse.json({
      stats: {
        ...
        totalContributions: mockDatasets.length + notebooks.length + mockModels.length + competitions.length
      },
      ...
      competitions,
      overview: [
        ...mockDatasets.slice(0, 5),
        ...notebooks.slice(0, 5),
        ...mockModels.slice(0, 5)
      ].sort((a, b) => new Date(b.updated).getTime() - new Date(a.updated).getTime()),
      activity: mockDatasets.slice(0, 5).map((d, i) => ({
        type: "Update",
        repo: d.title,
